...
import { updatePartnerDetails } from "../api/updateApi/updateApi";
...
const EditPartner = () => {
  ...
  const fetchPartnerDetails = async () => {
    try {
      ...
      if (response.success) {
        const partner = response.data.find((p: any) => String(p._id) === String(id));
        if (partner) {
          // Basic Details
          setName(partner.name || "");
          setPhone(partner.phone || "");
          setEmail(partner.email || "");
          setDob(partner.dateOfBirth ? new Date(partner.dateOfBirth) : new Date());
          setGender(partner.gender || "Male");

          // Payment System
          setDeliveryCharges(partner.paymentSystem?.deliveryCharges?.toString() || "");
          setOnTimeDeliveryBonus(partner.paymentSystem?.onTimeDeliveryBonus?.toString() || "");
          setHighRatingBonus(partner.paymentSystem?.highRatingBonus?.toString() || "");

          // Payment Details
          setBeneficiaryName(partner.paymentDetails?.beneficiaryName || "");
          setIfscCode(partner.paymentDetails?.ifscCode || "");
          setBankName(partner.paymentDetails?.bankName || "");
          setBankBranch(partner.paymentDetails?.bankBranch || "");
          setPanCardNo(partner.paymentDetails?.panCardNo || "");
          setAadharCardNo(partner.paymentDetails?.aadharCardNo || "");

          // Working Days
          if (partner.workingDays) {
            setWorkingDays(partner.workingDays);
          }

          // Set existing document previews if available
          if (partner.paymentDetails?.panCardUpload) {
            setPanCardPreview(partner.paymentDetails.panCardUpload);
          }
          if (partner.paymentDetails?.aadharCardUpload) {
            setAadharCardPreview(partner.paymentDetails.aadharCardUpload);
          }
        }
      } else {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (panCardFile) {
        const panCardFileObj = {
          uri: panCardFile.uri,
          type: 'image/jpeg',  
          name: panCardFile.name
        };
        formData.append('panCardUpload', panCardFileObj as any);
      }

      if (aadharCardFile) {
        const aadharCardFileObj = {
          uri: aadharCardFile.uri,
          type: 'image/jpeg',  
          name: aadharCardFile.name
        };
        formData.append('aadharCardUpload', aadharCardFileObj as any);
      }

      console.log('FormData contents:', Object.fromEntries(formData));

      if (id) {
        const result = await updatePartnerDetails(String(id), formData);
        if (result.success) {
      ...
      Alert.alert(
        "Error",
        error.message || "Failed to save partner details. Please try again."
      );
    } finally {
      setLoading(false);
    }
  ...
  // Mock ID images
  const idImages = [require("../../assets/images/chef.png"), require("../../assets/images/chef.png")];
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>
          {id ? "Edit Delivery Partner" : "Add Delivery Partner"}
        </Text>
        <View style={{ width: 24 }} />
      </View>
